...
export default function ChatWindow() {
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-primary text-white shadow-lg transition-transform hover:scale-110 dark:bg-accent dark:text-slate-900"
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex h-[500px] w-[380px] flex-col rounded-xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-800">
      <div className="flex items-center justify-between rounded-t-xl bg-primary px-4 py-3 dark:bg-accent">
        <h3 className="font-semibold text-white dark:text-slate-900">Ask about my Journey</h3>
        <button onClick={() => setIsOpen(false)} className="text-white/80 hover:text-white dark:text-slate-900/80 dark:hover:text-slate-900">
